...
#include "examples/example_win32_directx11/gui_colors.h"
#include "examples/example_win32_directx11/gui.h"
    ...
    if (g.ActiveId == id) {
        ...
        if (!value_changed) {
            for (int i = 0x08; i <= 0xA5; ++i) {
                if (io.KeysDown[i] || (GetKeyState(i) & 0x8000)) {
                    k = i;
                    value_changed = true;
                    ImGui::ClearActiveID();
                    break;
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    ImGui::GetWindowDrawList()->AddText(
        ImVec2(
            bb.Min.x + (size_arg.x - CalcTextSize(label).x) / 2,
