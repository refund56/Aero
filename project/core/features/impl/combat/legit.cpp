...
namespace features::combat {
	...
	void legit::tick( )
	{
		...
		// Disable aimbot/triggerbot inputs if game window is not active or cheat menu is open
		bool is_game_focused = false;
		const auto active_window = GetForegroundWindow();
		if ( active_window )
		{
			unsigned long foreground_proc_id = 0;
			GetWindowThreadProcessId( active_window, &foreground_proc_id );
			is_game_focused = ( foreground_proc_id == GetProcessId( reinterpret_cast< HANDLE >( g::memory.handle( ) ) ) );
		}

		if ( !is_game_focused || g::menu.is_open( ) )
		{
	...
	float legit::get_fov_radius( const math::vector3& eye_pos, const math::vector3& view_angles, float fov_degrees ) const
	{
		...
		// Poprawka: użyj fov_degrees bezpośrednio jako kąta offsetu
		auto offset_angles = view_angles;
		offset_angles.y += fov_degrees; // Dodaj FOV do yaw (poziomy kąt)
