...
namespace systems {
	...
	void collector::run( )
	{
		...
		this->m_update_count.fetch_add( 1, std::memory_order_release );
	}

	const std::vector<collector::player>& collector::players( ) const
	{
		thread_local std::vector<player> cached_players;
		thread_local std::uint64_t cached_update_count = 0xFFFFFFFF;

		const auto current_count = this->m_update_count.load( std::memory_order_acquire );
		if ( cached_update_count != current_count )
		{
			std::shared_lock lock( this->m_mutex );
			cached_players = this->m_players;
			cached_update_count = current_count;
		}
		return cached_players;
	}

	const std::vector<collector::item>& collector::items( ) const
	{
		thread_local std::vector<item> cached_items;
		thread_local std::uint64_t cached_update_count = 0xFFFFFFFF;
		...
		{
			std::shared_lock lock( this->m_mutex );
			cached_items = this->m_items;
			cached_update_count = current_count;
		}
		return cached_items;
	}

	const std::vector<collector::projectile>& collector::projectiles( ) const
	{
		thread_local std::vector<projectile> cached_projectiles;
		thread_local std::uint64_t cached_update_count = 0xFFFFFFFF;
		...
		{
			std::shared_lock lock( this->m_mutex );
			cached_projectiles = this->m_projectiles;
			cached_update_count = current_count;
		}
		return cached_projectiles;
	}
