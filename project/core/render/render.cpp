...
void render::run( )
{
	...
	while ( true )
	{
		...
		zdraw::begin_frame( );
		{
			...
			{
				game_hwnd = ::FindWindowA( "SDL_app", "Counter-Strike 2" );
			}

			const auto active_hwnd = ::GetForegroundWindow( );

			if ( game_hwnd == nullptr || active_hwnd == game_hwnd || active_hwnd == this->m_hwnd )
			{
				if ( systems::g_local.valid( ) )
				{
					...
					features::misc::g_nade_helper.on_render( draw_list );
					features::misc::g_radar.on_render( draw_list );
					features::misc::g_visual_extras.on_render( draw_list );
		...
		const UINT sync_interval = settings::g_misc.limit_fps ? 0 : 1;
		if ( FAILED( this->m_swap_chain->Present( sync_interval, 0 ) ) )
		{
			break;
		}
...
void render::update_input_window( )
{
	...
	if ( g::menu.is_open( ) )
	{
		if ( ( style & WS_EX_TRANSPARENT ) || ( style & WS_EX_NOACTIVATE ) )
		{
			...
			::SetWindowPos( this->m_hwnd, HWND_TOPMOST, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE | SWP_SHOWWINDOW | SWP_FRAMECHANGED );
...
bool render::setup_d3d( )
{
	...
	ui::g_pd3dDevice = this->m_device;
	ui::g_pd3dDeviceContext = this->m_context;
