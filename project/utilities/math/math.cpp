...
namespace math {
	...
	float helpers::calculate_fov( const vector3& view_angles, const vector3& src, const vector3& dst ) noexcept
	{
		...
		const auto delta_x = view_angles.x - angle.x;
		const auto delta_y = normalize_yaw( view_angles.y - angle.y );

		// Użyj długości wektora (euclidean distance) - standardowa metoda
